    ...
    return (
        <>
            ...
            <AdminPageShell
                ...
                actions={<Button asChild><Link href="/admin/testimonials/create">Tambah</Link></Button>}
            >
                <AdminTableCard
                    toolbar={<AdminSearch action="/admin/testimonials" defaultValue={filters.q} placeholder="Cari testimoni..." />}
                ...
                >
                    <AdminTable>
                        ...
                        <tbody>{testimonials.data.map((item) => <tr key={item.id} className="border-t"><td className="p-3"><div className="font-medium">{item.client_name}</div><div className="text-muted-foreground">{item.client_role ?? '-'}</div></td><td className="p-3">{item.rating ?? '-'}</td><td className="p-3">{item.is_featured ? 'Ya' : 'Tidak'}</td><td className="p-3 text-right"><Button variant="outline" size="sm" asChild><Link href={`/admin/testimonials/${item.id}/edit`}>Edit</Link></Button><Button variant="destructive" size="sm" onClick={() => confirm('Hapus testimoni ini?') && router.delete(`/admin/testimonials/${item.id}`)}>Hapus</Button></td></tr>)}</tbody>
                    </AdminTable>
                </AdminTableCard>
