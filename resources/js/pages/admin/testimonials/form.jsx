...
export default function TestimonialForm({ testimonial }) {
    ...
    function submit(e) {
        ...
        post(
            editing
                ? `/admin/testimonials/${testimonial.id}?_method=PUT`
                : '/admin/testimonials',
            { forceFormData: true },
        );
    }
    return (
        <>
            ...
            <form onSubmit={submit} className="max-w-2xl space-y-4 p-4">
                ...
                <Field label="Nama Klien" error={errors.client_name}>
                    <Input
                ...
                <Field label="Role Klien" error={errors.client_role}>
                    <Input
                        value={data.client_role}
                        onChange={(e) => setData('client_role', e.target.value)}
                    />
                </Field>
                <Field label="Foto Klien" error={errors.client_photo}>
                    <ImageUploadPreview
                        file={data.client_photo}
                        currentPath={testimonial?.client_photo}
                        alt={data.client_name || 'Preview testimoni'}
                    />
                    <Input
                        type="file"
                        accept="image/*"
                        onChange={(e) =>
                            setData('client_photo', e.target.files[0])
                        }
                    />
                </Field>
                <Field label="Isi Testimoni" error={errors.content}>
                    <textarea
