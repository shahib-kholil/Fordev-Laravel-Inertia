...
export default function PortfolioForm({ portfolio }) {
    ...
    function submit(e) {
        ...
        post(
            editing
                ? `/admin/portfolios/${portfolio.id}?_method=PUT`
                : '/admin/portfolios',
            { forceFormData: true },
        );
    }
        ...
        <>
            ...
            <form onSubmit={submit} className="max-w-2xl space-y-4 p-4">
                <h1 className="text-2xl font-semibold">
                    {editing ? 'Edit Portofolio' : 'Tambah Portofolio'}
                </h1>
                ...
                <Field label="Gambar" error={errors.image}>
                    <ImageUploadPreview
                        file={data.image}
                        currentPath={portfolio?.image}
                        alt={data.title || 'Preview portofolio'}
                    />
                    <Input
                        type="file"
                        accept="image/*"
                ...
                <Field label="URL Proyek" error={errors.project_url}>
                    <Input
                        type="url"
                ...
                <Field label="Urutan" error={errors.order_position}>
                    <Input
                        type="number"
                        min="0"
                        value={data.order_position}
                        onChange={(e) =>
                            setData('order_position', e.target.value)
                        }
                    />
                </Field>
...
PortfolioForm.layout = {
    breadcrumbs: [{ title: 'Portofolio', href: '/admin/portfolios' }],
};
